#!/usr/bin/env python3
"""
Model Training for London Historical LLM - Modern Version
Trains a transformer model on historical London texts using nanoGPT approach
"""
...
class GPTConfig:
    """GPT configuration for regular model"""
    def __init__(self, n_layer=24, n_head=16, n_embd=1024, block_size=1024, 
...
class GPT(torch.nn.Module):
    """GPT model based on nanoGPT - Regular size"""
...
class LondonHistoricalTrainer:
    ...
    def __init__(self, data_dir: str, tokenizer_dir: str, output_dir: str, resume_from_checkpoint: str = None):
        ...
        self.block_size = int(self.training_config["max_length"])
        self.learning_rate = self.training_config["learning_rate"]
        # Use max_steps if available, otherwise fall back to num_epochs * 1000
        if "max_steps" in self.training_config:
            self.max_iters = self.training_config["max_steps"]
        else:
            self.max_iters = self.training_config["num_epochs"] * 1000  # Convert epochs to steps
        self.eval_interval = self.training_config.get("eval_steps", 250)
        self.log_interval = self.training_config.get("logging_steps", 50)
        self.eval_iters = self.training_config.get("eval_iters", 100)
        
        # Model architecture - from config
        self.n_layer = self.training_config.get("n_layer", 24)
        self.n_head = self.training_config.get("n_head", 16)
        ...
        tf32 = self.training_config.get("enable_tf32", True)
        ...
        try:
            ...
            pass
        use_amp = self.training_config.get("enable_amp", True)
        amp_dtype_cfg = self.training_config.get("amp_dtype", "bf16").lower()
        ...
        if self.use_wandb:
            try:
                ...
                wandb.init(
                    ...
                    name=f"london-historical-llm-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
                    config=self.training_config,
                    tags=config.wandb_config["tags"] + ["regular"],
                    group=config.wandb_config["group"],
                    job_type=config.wandb_config["job_type"],
                    notes="London Historical LLM training with clean data"
                )
    ...
    def init_model(self):
        ...
        # Compile model
        if torch.cuda.is_available() and self.training_config.get("enable_compile", True):
    ...
    def resume_from_checkpoint_file(self):
        ...
        if not self.resume_from_checkpoint:
            ...
            if checkpoint_files:
                ...
                self.resume_from_checkpoint = str(latest_checkpoint)
            else:
                return
    ...
    def train(self):
        ...
        if self.master_process:
            ...
            # Final cleanup to ensure only last 5 checkpoints remain
            self.cleanup_old_checkpoints()
...
def main():
    parser = argparse.ArgumentParser(description='London Historical LLM Training - Regular Model')
